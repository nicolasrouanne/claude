...
| **slack-episto** | Stdio | `/opt/homebrew/bin/slack-mcp-server` | Slack Episto workspace — read channels, send messages, list users. Uses user token (`xoxp-`) to post as the authenticated user. Token: `SLACK_EPISTO_USER_TOKEN`. |
